...
def doExtraRadarSpreading(plate, emitter):
    # draw the emitter
    plate[emitter[0]][emitter[1]] = EXTRA_COVER

    # going up
    for j in range(1,4):
        if emitter[1] - j < 0 or plate[emitter[0]][emitter[1]] != 0:
            break
        else:
            plate[emitter[0]][emitter[1] - j] = EXTRA_COVER

    # going down
    for j in range(1,4):
        if emitter[1] + j >= number_N or plate[emitter[0]][emitter[1] + j] != 0:
            break
        else:
            plate[emitter[0]][emitter[1] + j] = EXTRA_COVER
    ...
    # going left
    for j in range(1,4):
        if emitter[0] - j < 0 or plate[emitter[0] - j][emitter[1]] != 0:
            break
    else:
        plate[emitter[0] - j][emitter[1]] = EXTRA_COVER
        
    # going right
    for j in range(1,4):
        if emitter[0] + j >= number_N or plate[emitter[0] - j][emitter[1]] != 0:
            break
        else:
            plate[emitter[0] + j][emitter[1]] = EXTRA_COVER
    ...
    # going left up
    for j in range(1,4):
        if emitter[1] - j < 0 or emitter[0] - j < 0 or plate[emitter[0] - j][emitter[1] - j] != 0:
            break
        else:
            plate[emitter[0] - j][emitter[1] - j] = EXTRA_COVER

    # going left down
    for j in range(1,4):
        if emitter[1] + j >= number_N or emitter[0] - j < 0 or plate[emitter[0] - j][emitter[1] + j] != 0:
            break
        else:
            plate[emitter[0] - j][emitter[1] + j] = EXTRA_COVER
    ...
    # going right up
    for j in range(1,4):
        if emitter[1] - j < 0 or emitter[0] + j >= number_N or plate[emitter[0] + j][emitter[1] - j] != 0:
            break
        else:
            plate[emitter[0] + j][emitter[1] - j] = EXTRA_COVER
    ...
    # going right down
    for j in range(1,4):
        if emitter[1] + j >= number_N or emitter[0] + j >= number_N or plate[emitter[0] + j][emitter[1] + j] != 0:
            break
        else:
            plate[emitter[0] + j][emitter[1] + j] = EXTRA_COVER
