...
class flight:
    ...
    # this function can only been used after assumed_landing_time and assumed_taking_off_time is initialized
...
def check_landing_gate(landing_time, gate_time):
    global global_L, global_G
    global assigned_plane
    ...
    for plane in assigned_plane:
        if plane.get_status(landing_time) == LANDING:
            count_L += 1
        if plane.get_status(gate_time) == AT_GATE:
            count_G += 1
        if plane.get_status((landing_time + gate_time) / 2) == LANDING:
            count_L_middle += 1
        if plane.get_status(gate_time - 1) == LANDING:
            count_L_end += 1

    if count_L >= global_L or count_G >= global_G or count_L_middle >= global_L or count_L_end >= global_L:
        return False
    else:
        return True


def check_taking_off(taking_off_time):
    global global_T
    global assigned_plane
    ...
    for plane in assigned_plane:
        if plane.get_status(taking_off_time) == TAKING_OFF:
            count_T += 1
...
def back_track():
    ...
    '''
    if len(list_plane) == 0 and check_constraints() == False:
        return False
    '''

    plane = list_plane.pop()
    for va in plane.landing_domain:
        plane.set_landing_time(va)
        plane.set_taking_off_bound()


        if not check_landing_gate(va, va + plane.M):
            continue

        for vb in plane.taking_off_domain:
            if not check_taking_off(vb):
                continue
...
def main():
    ...
    with open('input4.txt', 'r') as inputFile:
        ...
        for line in lines:
            ...
            list_plane.append(
                flight(int(temp_line[0]), int(temp_line[1]), int(temp_line[2]), int(temp_line[3]), int(temp_line[4]),
                       i))
            i += 1
